...
runlog = pd.DataFrame(columns=['name', 'epoch_time', 'type'])
...
for i in range(5, 15):
    ...
    for step, data in enumerate(data1):
        ...
        time = data[2]
        type = "with_OptNet"
        r = {'name': name, 'epoch_time': time, 'type': type}
...
sns.displot(data=runlog, x="epoch_time", hue='type', kind="kde",  fill=True )
plt.title("distribution epoch time")
