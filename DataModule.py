...
class DataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        ...
        for i in range(self.system.ns):
            s_i_grid = torch.arange(domain_lower_bd[i], domain_upper_bd[i], self.grid_gap)
            s_grid_list.append(s_i_grid.float())
        
        
        mesh_grids = torch.meshgrid(s_grid_list)
        unsqueez_mesh_grid = [ torch.unsqueeze(mesh_grid, dim=0) for mesh_grid in mesh_grids ]
        mesh_grids = torch.vstack(unsqueez_mesh_grid)
        data_points = torch.flatten(mesh_grids, start_dim=1)
        s_samples = data_points.T
        ...
        self.safe_mask_training = self.s_training.norm(dim=-1) <= 1
        self.unsafe_mask_training = self.s_training.norm(dim=-1) >= 1.2#self.system.unsafe_mask(self.s_training)

        self.safe_mask_validation = self.s_validation.norm(dim=-1) <= 1
