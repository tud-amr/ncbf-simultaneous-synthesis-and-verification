...
from safe_rl_cbf.RL.DubinsCarRotate.DubinsCarRotate import DubinsCarRotate

class DubinsCarRotateEnv(gym.Env):
    def __init__(self, render_sim=False, v= 1):
        """
        ...
        self.state = np.array([self.x_init, self.y_init, self.theta_init])

        # action property
        self.max_omega = 1.0
        self.action_space = spaces.Box(low=np.array([-self.max_omega]), high=np.array([self.max_omega]), dtype=np.float32)
    ...
    def init_agent(self):
        self.car = DubinsCarRotate(1, self.radius, self.v, self.state, self.space, self.scale, self.dt)
