...
for i in range(0, 1):
    ...
    log_dir = "logs/stable_baseline_logs/cartpole/" + "run" + str(i) + "/"
    ...
    model = PPO("MlpPolicy", my_cp_env, verbose=1, tensorboard_log=log_dir )

    model.learn(total_timesteps=800000, callback=custom_cb, tb_log_name= my_cp_env.prefix)
    model.save(log_dir + "cartpole_without_CBF")
