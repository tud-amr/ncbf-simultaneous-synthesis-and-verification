...
#     model.learn(total_timesteps=10000, callback=custom_cb, tb_log_name= env.prefix)
...
for i in range(0, 2):
    ...
    data_module = TrainingDataModule(system=system, val_split=0, train_batch_size=512, training_points_num=int(1e6))
    NN = NeuralNetwork.load_from_checkpoint("logs/CBF_logs/point_robot_new_2/lightning_logs/version_21/checkpoints/epoch=4-step=20.ckpt", dynamic_system=system, data_module=data_module )
    # NN.to(device)
